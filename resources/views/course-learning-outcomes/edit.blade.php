    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route(syllabi.ilos.clos.update) }}" method="post">
                    <div class="pb-4">
                        <div class="pb-2"><label for="position"><strong class="font-semibold text-gray-900 dark:text-white">Position</strong></label></div>
                        <input type="text" placeholder="Posisi CLO" class="input input-bordered w-full max-w-xs" value="{{ $clo->position }}"/> <br>
                    </div>
                    <div class="pb-4">
                    <div class="pb-2"><label for="position"><strong class="font-semibold text-gray-900 dark:text-white">Description</strong></label></div>
                        <input type="textarea" class="textarea textarea-bordered w-full" placeholder="Deskripsi CLO" value="{{ $clo->description }}"></textarea>
                    </div>
                    <input type="submit" value="Save" class="btn btn-active rounded-md " />
                </form>
                <form action="{{ route(syllabi.ilos.clos.update) }}">
